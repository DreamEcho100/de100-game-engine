...
get_recording_filename(const char *exe_directory, int32 slot_index,
                       char *buffer, size_t buffer_size) {
  // slot_index is used to support multiple recording slots.
  snprintf(buffer, buffer_size, "%sloop_edit_%d.hmi", exe_directory,
           slot_index);
}
...
bool input_recording_begin(const char *exe_directory, GameMemoryState *state,
  ...
  char filename[256];
  get_recording_filename(exe_directory, slot_index, filename, sizeof(filename));
...
void input_recording_toggle(const char *exe_directory, GameMemoryState *state) {
  if (input_recording_is_playing(state)) {
    ...
    input_recording_begin(exe_directory, state, 1);
  }
}
