...
**How:** Add `bool paused` to `GameState`. In platform loop: `if (!state.paused) snake_update(...)`. In `snake_render()`: if paused, call `draw_rect_blend(bb, 0, HEADER_ROWS * CELL_SIZE, bb->width, GRID_HEIGHT * CELL_SIZE, GAME_RGBA(0,0,0,160))` then draw "PAUSED" text centered.
