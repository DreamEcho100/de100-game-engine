...
static void draw_cell(int col, int row, unsigned long color) {
  ...
  XFillRectangle(g_display, g_window, g_gc, col * CELL_SIZE + 1, /* x pixel */
 ...
 *  We redraw everything every frame (no dirty-region tracking needed at this
 * scale). ─────────────────────────────────────────────────────────── */
void platform_render(const GameState *state) {
  ...
  XFillRectangle(g_display, g_window, g_gc, 0, 0, WINDOW_WIDTH,
                 HEADER_ROWS * CELL_SIZE);

  /* 3. Header bottom border line (row HEADER_ROWS-1, full width, green) */
  XSetForeground(g_display, g_gc, g_col[1]);
  XFillRectangle(g_display, g_window, g_gc, 0, (HEADER_ROWS - 1) * CELL_SIZE,
                 WINDOW_WIDTH, 2);
  ...
  XDrawString(g_display, g_window, g_gc, 10, CELL_SIZE + CELL_SIZE / 2, buf,
              (int)strlen(buf));
  snprintf(buf, sizeof(buf), "Best: %d", state->best_score);
  XDrawString(g_display, g_window, g_gc, WINDOW_WIDTH - 80,
              CELL_SIZE + CELL_SIZE / 2, buf, (int)strlen(buf));
  ...
  /* 9. Draw snake head */
  draw_cell(state->segments[state->head].x, state->segments[state->head].y,
            head_color);
  ...
  if (state->game_over) {
    ...
    XSetForeground(g_display, g_gc, g_col[6]);
    snprintf(buf, sizeof(buf), "Score: %d", state->score);
    XDrawString(g_display, g_window, g_gc, cx - 24, cy + 8, buf,
                (int)strlen(buf));
