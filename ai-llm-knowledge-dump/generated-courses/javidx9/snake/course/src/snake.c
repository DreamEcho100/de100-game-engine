...
static void draw_rect_blend(SnakeBackbuffer *bb, int x, int y, int w, int h,
    ...
    for (py = y0; py < y1; py++) {
        for (px = x0; px < x1; px++) {
            ...
            *dst = SNAKE_RGB(out_r, out_g, out_b);
        }
    }
}
...
static void draw_char(SnakeBackbuffer *bb, int x, int y, char c,
    ...
    if      (c >= '0' && c <= '9') bitmap = FONT_DIGITS[c - '0'];
    else if (c >= 'A' && c <= 'Z') bitmap = FONT_LETTERS[c - 'A'];
    else if (c >= 'a' && c <= 'z') bitmap = FONT_LETTERS[c - 'a'];
    else if (c == ' ')             return;  /* space = skip */
    else {
...
void snake_render(const GameState *s, SnakeBackbuffer *bb) {
    ...
    /* 4. Header text: "SNAKE" centered, score left, best right */
    {
        ...
        draw_text(bb, bb->width - (int)(strlen(buf)) * 12 - 8, text_y,
                  buf, COLOR_YELLOW, 2);
    }
    ...
    if (s->game_over) {
        ...
        draw_rect_blend(bb, 0, field_y, field_w, field_h,
