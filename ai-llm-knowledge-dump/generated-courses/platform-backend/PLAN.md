...
**Why `platform_swap_input_buffers` in `game/base.h`?** The function swaps two `GameInput` structs — it is purely game-layer logic with no OS dependencies. Placing it next to `prepare_input_frame` (which it is always paired with) keeps the architecture boundary clean: `platform.h` declares only functions that backends *implement*, while `game/base.h` declares helpers that backends *call*.
